...
void LogSum::DoBuild() {
  ...
  if (second_estimate_->transform_for_objective()) {
    ...
    if (second_estimate_->value() < second_estimate_->lower_bound() || second_estimate_->value() > second_estimate_->upper_bound())
      LOG_ERROR_P(PARAM_THETA_TWO) << " " << PARAM_PRIOR_APPLIES_TO_TRANSFORM << " was set to true, but the transformed parameter = " << second_estimate_->value()
    ...
    if (estimate_->value() < estimate_->lower_bound() || estimate_->value() > estimate_->upper_bound())
      LOG_ERROR_P(PARAM_THETA_ONE) << " " << PARAM_PRIOR_APPLIES_TO_TRANSFORM << " was set to true, but the transformed parameter = " << estimate_->value()
                                   << " which is outside of these bounds. Please check the bounds.";
    Restore();
  }
}
