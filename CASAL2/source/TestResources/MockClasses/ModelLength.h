 ...
 */
#ifndef MOCK_MODEL_LENGTH_H_
#define MOCK_MODEL_LENGTH_H_
#ifdef TESTMODE
 ...
 * Class Definition
 */
class MockModelLength : public niwa::model::Length, public std::enable_shared_from_this<MockModelLength> {
public:
  // Constructor
  MockModelLength() {
    set_start_year(1990);
    ...
    set_length_midpoints({15, 25, 35, 45});
    set_length_plus(false);
    set_number_of_length_bins();
  }

  virtual ~MockModelLength() = default;
  ...
  MOCK_CONST_METHOD0(length_bin_mid_points, vector<double>&());
  ...
  void set_length_midpoints(vector<double> model_length_mid_points) { model_length_bin_mid_points_ = model_length_mid_points; }
  ...
  void bind_calls() {
    ...
    EXPECT_CALL(*this, length_bin_mid_points()).WillRepeatedly(ReturnRef(model_length_bin_mid_points_));
  }
  ...
  void bind_calls_to_default() {
    ...
    EXPECT_CALL(*this, length_bin_mid_points()).WillRepeatedly(DoDefault());
