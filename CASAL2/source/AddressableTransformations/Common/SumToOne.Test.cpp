 ...
 * Sample unit test
 */
TEST_F(InternalEmptyModel, AddressableTransformation_SumToOne) {
  const string simple_difference_for_twosex =
      R"(
      @parameter_transformation transform
      type difference
  ...
  auto difference_parameter = model_->objects().GetAddressable("parameter_transformation[transform].difference_parameter");
 ...
 *
 */
TEST_F(InternalEmptyModel, AddressableTransformation_SumToOne_Bad) {
