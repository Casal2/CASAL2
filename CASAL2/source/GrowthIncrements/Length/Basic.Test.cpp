...
public:
  MockBasic(shared_ptr<Model> model, Double g_a, Double g_b, Double l_a, Double l_b) : Basic(model) {
    g_a_ = g_a;
    g_b_ = g_b;
    l_a_ = l_a;
    l_b_ = l_b;
...
TEST(GrowthIncrements, Basic) {
  ...
  for (unsigned i = 0; i < lengths.size(); i++) {
    EXPECT_NEAR(basic.Mockget_value(lengths[i]), result[i], 1e-3) << " i " << i << " get _value = " << basic.Mockget_value(lengths[i]) << " result " << result[i];
  }
}
