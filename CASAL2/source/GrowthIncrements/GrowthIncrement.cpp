      ...
      for(unsigned i = 0; i < number_of_model_length_bins_; ++i)  {
        if((i == (number_of_model_length_bins_ - 1)) & plus_group_) {
          ...
          if (plus_group_){
            ...
            LOG_FINEST() << "pnorm = " << utilities::math::pnorm2(model_min_length_bins_[number_of_model_length_bins_] - model_length_midpoints_[i], mu, sigma) << " value = " << model_min_length_bins_[number_of_model_length_bins_] << " midpoint " << model_length_midpoints_[i];
