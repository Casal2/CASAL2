 ...
 * Default constructor
 */
GrowthBasic::GrowthBasic(shared_ptr<Model> model) : Process(model), partition_(model) {
...
void GrowthBasic::DoValidate() {
  if (growth_time_steps_.size() != n_growth_episodes_)
    LOG_ERROR_P(PARAM_GROWTH_TIME_STEPS) << "You supplied " << growth_time_steps_.size() << " time step labels but only have " << n_growth_episodes_
                                         << " in the model. These need to be the same";
}
...
void GrowthBasic::DoReset() {
  ...
  for (unsigned length_bin = 0; length_bin < length_bins_.size(); ++length_bin) {
    ...
    transition_matrix_[length_bin][length_bin]
        = math::pnorm(length_bins_[length_bin + 1] - length_bin_mid_points_[length_bin], mu, sigma);  // TODO: Check if calculating the normal PDF using Boost works with autodiff
