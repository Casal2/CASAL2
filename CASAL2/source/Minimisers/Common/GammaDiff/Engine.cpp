...
//**********************************************************************
double Engine::optimise_finite_differences(gammadiff::CallBack& objective, vector<double>& StartValues, vector<double>& LowerBounds, vector<double>& UpperBounds, int& convergence,
  ...
  while (clMinimiser.getResult() >= 0) {
    ...
    if ((clMinimiser.getResult() == 0) || (clMinimiser.getResult() == 2)) {
      ...
      for (int i = 0; i < iVectorSize; ++i) {
        sValues = sValues + std::to_string(vCurrentValues[i]) + " ";
      }
      LOG_INFO() << "Objective function value: " << dScore;
      LOG_INFO() << "About to evaluate function with x = " << sValues;
    }
