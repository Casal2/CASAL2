  ...
  if (model_->state() == State::kInitialise) {
    ...
    if (!use_age_weights_) {
      ...
      for (unsigned i = 0; i < partition_.size() && iterator != partition_.end(); ++i, ++iterator) {
        for (unsigned j = 0; j < (*iterator)->data_.size(); ++j) {
          unsigned age = (*iterator)->min_age_ + j;
          LOG_FINE() << "Biomass for category = " << (*iterator)->name_ << " age = " << age << " mean weight = " << (*iterator)->age_length_->mean_weight(time_step_index, age)
                     << " selectivity = " << selectivities_[i]->GetAgeResult(age, (*iterator)->age_length_) << " numbers = " << (*iterator)->data_[j];
      ...
      for (unsigned i = 0; i < partition_.size() && iterator != partition_.end(); ++i, ++iterator) {
        for (unsigned j = 0; j < (*iterator)->data_.size(); ++j) {
          unsigned age = (*iterator)->min_age_ + j;
          LOG_FINE() << "Biomass for category = " << (*iterator)->name_ << " age = " << age << " mean weight = " << age_weights_[i]->mean_weight_at_age_by_year(year, age)
                     << " selectivity = " << selectivities_[i]->GetAgeResult(age, (*iterator)->age_length_) << " numbers = " << (*iterator)->data_[j];
