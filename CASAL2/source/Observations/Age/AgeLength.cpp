...
AgeLength::AgeLength(shared_ptr<Model> model) : Observation(model) {
  ...
  parameters_.Bind<double>(PARAM_AGES, &ages_, "Vector of individual ages", "", "");
  parameters_.Bind<double>(PARAM_LENGTHS, &lengths_, "Vector of individual lengths that correspond in a one to one relationship with ages", "", "");
  parameters_.Bind<string>(PARAM_SAMPLING_TYPE, &sample_type_, "The sampling type used to collect this data", "", "")->set_allowed_values({PARAM_AGE, PARAM_LENGTH, PARAM_RANDOM});
...
void AgeLength::DoValidate() {
  ...
  if(ages_.size() != lengths_.size())
    LOG_ERROR_P(PARAM_AGES) << " needs to have the same number of elements as " << PARAM_LENGTHS << ". '" << ages_.size() << "' ages supplied but '" << lengths_.size() << "' lengths.";
  ...
  if(years_.size()> 1) 
    LOG_ERROR_P(PARAM_YEARS) << "AgeLength observations can only be supplied for one year";
  if(sample_type_ == PARAM_AGE) {
    ...
    for(unsigned i = 0; i < lengths_.size(); i++) {
      if(!utilities::math::in(unique_lengths_,lengths_[i])) {
        unique_lengths_.push_back(lengths_[i]);
      }
    }
  ...
  for(unsigned i = 0; i < n_fish_; i++) {
    if(ages_[i] < model_->min_age())
      LOG_ERROR_P(PARAM_AGES) << " age '" << ages_[i] << "' at element '" << i + 1 << "' is less than the model min_age. This is not allowed.";
    if(ages_[i] > model_->max_age())
      LOG_ERROR_P(PARAM_AGES) << " age '" << ages_[i] << "' at element '" << i + 1 << "' is greater than the model max_age. This is not allowed.";
  }
...
void AgeLength::Execute() {
  ...
  /**
   * Loop through the provided categories and get numbers at age
   */
      ...
      for (unsigned cached_counter = 0; category_iter != partition_iter->end();  ++category_iter, ++cached_counter) {
        ...
        if(selectivities_[cached_counter]->IsSelectivityLengthBased()) {
          for(unsigned i = 0; i < n_fish_; ++i) {
            for(unsigned age_iter = 0;age_iter < age_spread_; ++age_iter) {
              ...
              pdf_by_age_and_length_[cached_counter][i] += numbers_at_age_[cached_counter][age_iter] * (*category_iter)->age_length_->get_pdf_with_sized_based_selectivity(age, lengths_[i], year, time_step, selectivities_[cached_counter]);
            }
          }
        } else {
          for(unsigned i = 0; i < n_fish_; ++i) {
            for(unsigned age_iter = 0;age_iter < age_spread_; ++age_iter) {
              unsigned age       = model_->min_age() + age_iter;
              pdf_by_age_and_length_[cached_counter][i] += numbers_at_age_[cached_counter][age_iter] * (*category_iter)->age_length_->get_pdf(age, lengths_[i], year, time_step);
            }
          }
        }
        ...
        for(unsigned i = 0; i < n_fish_; ++i) {
          if(ageing_error_) {
            ...
            for(unsigned age_iter = 0;age_iter < age_spread_; ++age_iter) {
              if(mis_matrix[age_iter][(unsigned)ages_[i] - model_->min_age()] > 1e-4) {
                if(selectivities_[cached_counter]->IsSelectivityLengthBased()) {
                  numerator +=  numbers_at_age_[cached_counter][age_iter] * mis_matrix[age_iter][(unsigned)ages_[i] - model_->min_age()] * (*category_iter)->age_length_->get_pdf_with_sized_based_selectivity((unsigned)ages_[i], lengths_[i], year, time_step,selectivities_[cached_counter]);
                } else {
                  numerator +=  numbers_at_age_[cached_counter][age_iter] * mis_matrix[age_iter][(unsigned)ages_[i] - model_->min_age()] * (*category_iter)->age_length_->get_pdf((unsigned)ages_[i], lengths_[i], year, time_step);
                }
              }
            }
          } else {
            ...
            if(selectivities_[cached_counter]->IsSelectivityLengthBased()) {
              numerator =  numbers_at_age_[cached_counter][(unsigned)ages_[i] - model_->min_age()] * (*category_iter)->age_length_->get_pdf_with_sized_based_selectivity((unsigned)ages_[i], lengths_[i], year, time_step,selectivities_[cached_counter]);
            } else {
              numerator =  numbers_at_age_[cached_counter][(unsigned)ages_[i] - model_->min_age()] * (*category_iter)->age_length_->get_pdf((unsigned)ages_[i], lengths_[i], year, time_step);
            }
          }
        }
