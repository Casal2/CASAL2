...
\CH
The \CASName\ (\CAS) is an age-structured population dynamics model. It implements a statistical catch-at-age population dynamics, using a discrete time-step state-space model that represents a cohort-based population age structure \index{About \CAS}. 

\CAS\ is run from the console window on Microsoft Windows or from a terminal window on Linux. \CAS gets its information from input data files, the main one of which is the \emph{\config}. Commands and subcommands in the \config\ are used to define the model structure, provide observations, define parameters, and define the outputs (reports) for \CAS. Command line switches tell \CAS\ the run mode and where to direct its output. See Section \ref{sec:running-sam} for the details.
