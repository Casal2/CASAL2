...
\title{\iSAMName\ User Manual \\\iSAM\ \VER} %Document title
\author{A. Dunn, I. Doonan, S. Rasmussen, K. Large} %Document authors %check this
...
\usepackage[bookmarks=true,bookmarksopen=false,bookmarksnumbered=true,
            ...
            pdfauthor={Alistair Dunn, Ian Doonan, Scott Rasmussen, Kath Large} %author list required
            pdfsubject={Stock Assessment Model Users manual},
            pdftex]{hyperref}
\hypersetup{
...
%for now \iSAM is {iSAM}, \iSAMName is {Stock Assessment Model}
%\iSAM and \iSAMName will not change, but {} will.
\newcommand{\VER}{\SourceControlShortVersionDoc} % SAM program version
\newcommand{\iSAM}{\texttt{iSAM}} % SAM short name
\newcommand{\isam}{\texttt{isam}} % sam prefix/suffix
\newcommand{\iSAMName}{Integrated Stock Assessment Model} %SAM long name
...
\newcommand{\ManualRef}{Dunn A., Doonan I., Rasmussen S., Large, K. (\DocYear) \iSAMName\ User Manual, \iSAM\ \VER. \Organisation\ \emph{Unpublished report}. \ref{TotPages} p.} % full document reference
...
% Title page
\pdfbookmark[1]{iSAM Users Manual}{title}
...
%\iSAM\ logo being developed by Erika MacKay.\\\\
%SAM\ name tbd.\\\\
...
%from the \iSAM\ code for Population, Estimation, Observation and
%Report sections.\\\\
...
~\vfill
\begin{center}
  \iSAMName\ User Manual (modified: \DocVer) \\ for use with \iSAM\ \VER
\end{center}
