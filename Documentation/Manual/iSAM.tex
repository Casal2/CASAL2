...
\title{\Huge\textbf{\CAS} \Large \\\CASName \\\CAS\ \VER} %Document title
...
\usepackage[bookmarks=true,bookmarksopen=false,bookmarksnumbered=true,
            ...
            pdftitle={C++ algorithmic stock assessment laboratory 2 (CASAL2) User Manual}, %title to be confimed
            pdfauthor={Alistair Dunn, Ian Doonan, Scott Rasmussen, Kath Large} %author list required
            pdfsubject={Stock Assessment Model User manual},
            pdftex]{hyperref}
\hypersetup{
...
\definecolor{new}{rgb}{0.5078,0.803922,0.196078}
...
\newcommand{\CAS}{$\texttt{CASAL}^{\texttt{2}}$} % CASAL2 short name
\newcommand{\cas}{$\texttt{casal}^{\texttt{2}}$} % CASAL2 prefix/suffix
\newcommand{\CASName}{C++ Stock Assessment Laboratory} %CASAL2 long name
...
\newcommand{\ManualRef}{Dunn A., Doonan I., Rasmussen S., Large, K. (\DocYear) \CASName\ User Manual, \CAS\ \VER. \Organisation\ \emph{Unpublished report}. \ref{TotPages} p.} % full document reference
...
~\vfill
\begin{center}
  \CASName\ User Manual (modified: \DocVer) \\ for use with \CAS\ \VER
\end{center}
