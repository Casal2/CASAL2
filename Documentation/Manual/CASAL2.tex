...
\newcommand{\email}{casal@niwa.co.nz}
...
%% Allow for a draft statement at beginning of manual
\usepackage{type1cm}
\usepackage{eso-pic}
\makeatletter
\AddToShipoutPicture*{%
...
\begin{titlepage}
  \thispagestyle{empty} % no header/footer/page number on this page
	\begin{center}
		...
		\begin{figure}[htp]
			%\begin{left}
			 \includegraphics[height=3cm]{Figures/CASAL2.png}
			%\end{left}
		\end{figure}
		...
		\vspace{1.0cm}
		\Large\CNAME\ User Manual (modified \DocVer) for use\\with \cname\texttt{\_\VER}
