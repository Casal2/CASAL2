...
The categories can include combinations of levels from one or more  factors such as sex, maturity state, area, or even species. \CNAME~has no predefined categories; \emph{all} categories are defined by the user.
...
Example of specifying categories using the shortcuts:
...
To estimate a common value for a block of years in a map parameter we use the \textit{same} subcommand. We illustrate the idea within the process \command{time\_varying}\texttt{[label].type=constant},where we want to fix \textit{q} over a specified block of years, 1992 to 1995.

We first specify the relationships in a \command{time\_varying} block:
...
Next, to estimate only one \textit{q} value for the time block, we pick one element of the map (say 1992), and then force all other years to have the same value, e.g.,  
...
\subsection{\I{Model scope and structure}}\label{sub:sec:pop_sec}
...
This would be specified using @time\_step block, i.e.
...
\paragraph{\I{Cinitial Initialisation UNTESTED}}
...
\paragraph{\I{
State\_category\_by\_age UNTESTED}}
...
\paragraph{\I{Timing evaluation interval; timing the point when observations are fitted or derived quantities are evaluated }}\label{sec:mortality_block}\label{Timing evaluation interval}
...
#### GO OVER M and specifing M-by-age HERE FOR ALL M-PROCESSES###########
...
\paragraph{Holling mortality rate UNTESTED}
...
\subsubsection{\I{Tag Release events UNTESTED}}\label{sub:tag_release}
...
\subsubsection{\I{Tag Loss UNTESTED}}
...
\subsection{\I{Age-weight relationship UNTESTED}\label{sec:weight-at-age}}
...
\subsection{\I{Weightless model UNTESTED}\label{sec:weightless-model}}
...
\subsubsection[Knife-edge]{\argument{knife\_edge}  UNTESTED}
\begin{equation}
f(x)= \begin{cases}
  0, & \text{if $x < E$} \\
  \alpha, & \text{if $x \ge E$}\\
...
\subsubsection[Increasing ]{\argument{increasing} UNTESTED?}\index{Selectivities!Increasing}
...
\subsubsection[Inverse logistic]{\argument{inverse\_logistic} UNTESTED??}\index{Selectivities!Inverse-logistic }
...
\subsection{\I{Time-varying Parameters UNTESTED}\label{sec:time_var}}
...
\subsection{\I{Equation Parser  UNTESTED NOT SURE HOW THIS WORKS}\label{sec:eq_parser}}
...
\subsection{\I{Specifying projections UNTESTED, BUT BEING DONE?}}\label{sec:projection}
...
Parameter values for the projected years can be specified in a stochastic way or fixed at some value (the default is the estimated value if the parameter is not time-varying) and these are specified in the \textit{@projection} block, e.g.,
...
Future catches are also specified in a \textit{@project} block, one for each fishery (see \ref{Projections@Catches} for examples). Here, a fishery is reference in the \textit{parameter}
...
\paragraph[Lognormal]{Sampling from a lognormal distribution, type  \argument{lognormal} UNTESTED}\index{Projections!Lognormal}
...
\paragraph[Lognormal-Empirical]{Sampling from a lognormal distribution where the  variance is estimated from values over a specified year range, type  \argument{lognormal\_empirical} UNTESTED}\index{Projections!Lognormal-Empirical}
...
\paragraph[User Defined]{Sample from a user-defined function, \argument{user\_defined}  UNTESTED}\index{Projections!User Defined}
