...
Calculate the ($P_{l,j}$) proportion of this tagged fish that will be moved from this each source category

$$P_{l,j} = N_{l,j} / T_l$$


Calculate the category specific transition rate for this length bin ($u_l$)

$$u_{l,j} = N_l / T_l * P_{l,j}$$
...
u_{l,j} = 
\begin{cases}
u_{max},& \text{if } u_{l,j} > u_{max} \text{ \textbf{flag a penalty}}\\
u_{l,j},  & \text{otherwise}
\end{cases}
\]

Calculate the numbers at length in this category that we are going to move by multiplying across the age-length matrix and storing this info by age, for each age we want to accumulated these for all length bins. then move the necceassary 

$$N_{a,j} += N_{a,l} * u_l * P_{l,j}$$
