...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.3]{Figures/annual_cycle.jpg}
...
For every time step in an annual cycle there is an associated mortality block. As mentioned earlier a mortality block is a consecutive sequence of mortality processes. These include all the processes described under subsection~\ref{sec:mortality}. Each time step can have at most one mortality block, and if no mortality processes occur in a time step then it defaults to the end of the time step. \CNAME\ will error out if the user defines multiple mortality processes in a single time step, that are not consecutive processes. Mortality blocks are related to derived quantities

Mortality blocks are an important concept in the model structure, and is used for derived quantities (see Section~\ref{sec:derived-quantities}) and observations (see Section~\ref{sec:observation-section}).
...
Parameters that are of type vector or map can be re-sampled with replacement between a year range, for use as projected year values.
