...
\end{itemize}
We believe the \subcommand{age} option are quite unlikely to be true, yet they are widely applied in fisheries (for example, in the Coleraine \citep{hilborn2001coleraine} stock modelling software). Probably the \subcommand{length} option are most likely to hold for most New Zealand finfish programmes.
...
where the effective sample size is a linear function of input sample size with intercept \((1 + \theta)^{-1}\)  and slope \(\frac{\theta}{1 + \theta}\).
...
If you estimate \(\theta\) it is recommended to apply a transformation such as log (see below for example syntax). This likelihood is quite different to configure compared with other likelihood types because it has an estimable parameter you need to define a \command{likelihood} block for.
