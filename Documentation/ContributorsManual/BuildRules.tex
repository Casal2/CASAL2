...
When generating a new report or adapting existing reports remember to use the \argument{AS\_DOUBLE()} if you are reporting a value of type \argument{double}. The reason behind this is so all minimisers don't add garbage to the end. An example of this will be when you use the adolc minimiser. If you haven't used the \argument{AS\_DOUBLE()} the report will have values trailed with an (a) e.g. 2.43(a). This will break the \R\ comparability.
