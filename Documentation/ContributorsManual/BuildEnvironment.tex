...
{\color{red}I think "gfortran" should be "fortran" here. To make it clearer, probably replace the sentence with "Ensure you have the "fortran" and "openmp" options installed as a part of the "gcc" dropdown tickboxes otherwise \CNAME will not compile.".}
...
It's possible {\color{red}that} there will be build errors or issues building the third-party libraries. If you encounter an error{\color{red},} then it’s worth checking the log files. Each third-party build system stores a log of everything it’s doing. The files will be named
