\section{Running \SAM\label{sec:running-sam}\index{Running \SAM}}
\CH
\SAM\ is run from the console window (i.e., the DOS command line) on \I{Microsoft Windows} or from a terminal window on \I{Linux}. \SAM\ gets its information from input data files, the key one of which is the \config\index{Input configuration file}. 
...
\CH
\SAM\ uses the standard output stream \texttt{standard output}\index{standard output} to display run-time information. The \I{standard error} stream is used by \SAM\ to output the program exit status and run-time errors. We suggest redirecting both the standard output and standard error into files\index{Redirecting standard out}\index{Redirecting standard error}. With the bash shell (on Linux systems), you can do this using the command structure,
...
\item [\texttt{-v}] Display the \SAM\ version number.
...
\item [\texttt{-q}] Run \emph{quietly}, i.e., suppress verbose printing of \SAM.

\item [\texttt{-g \emph{seed}}]  Seed the random number \emph{generator} with \texttt{\emph{seed}}, a positive (long) integer value. Note, if \texttt{-g} is not specified, then \SAM\ will  generate a random number seed based on the computer clock time.
\end{description}

\subsection{Constructing an \SAM\ \config \label{constructing-spm-config}}\index{Input configuration file syntax}
\CH
...
\subsection{\SAM\ exit status values\index{Exit status value}}
\CH
