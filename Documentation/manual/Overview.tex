...
\SAM\ is run from the console window on Microsoft Windows or from a terminal window on Linux. \SAM gets its information from input data files, the main one of which is the \emph{\config}. Commands and subcommands in the \config\ are used to define the model structure, provide observations, define parameters, and define the outputs (reports) for \SAM. Command line switches tell \SAM\ the run mode and where to direct its output. See Section \ref{sec:running-spm} for the details.
