...
\usepackage[bookmarks=true,bookmarksopen=false,bookmarksnumbered=true,
            ...
            pdftitle={Integrated Stock Assessment Modelling (aiSAM) Users Manual}, %title to be confimed
            pdfauthor={Kermit the Frog; Fozzie Bear}, %author list required
            pdfsubject={Stock Assessment Model},
            pdftex]{hyperref}
\hypersetup{
...
% command shortcuts: copied from SPM, setup needs defining
...
\newcommand{\COO}{\color{red}(\emph{was NYI, to be included?})\color{black}}
\newcommand{\CA}{\color{red}(\emph{following from CASAL, to be included?})\color{black}}
\newcommand{\CH}{\color{red}\emph{check and confirm text\\}\color{black}}
\newcommand{\EX}{\\\color{red}\emph{need an example here...}}
\newcommand{\EXend}{\color{black}}
\newcommand{\KL}{\color{red}\emph{KL comment: }}
\newcommand{\KLend}{\color{black}}


%quick index: copied from SPM, setup needs defining
...
% Input iSAM version definitions
\input{version.tex}
...
%iSAM cannot be used officially, so new name required!!! WHAT IS IT!
%for now \SAM is {iSAM}, \SAMName is {Stock Assessment Model}
%\SAM and \SAMName will not change, but {} will.
\newcommand{\VER}{v\input{SAMversion.tex}} % SAM program version
\newcommand{\SAM}{\texttt{SAM}} % SAM short name
\newcommand{\sam}{\texttt{sam}} % sam prefix/suffix
\newcommand{\SAMName}{Stock Assessment Model} %SAM long name
\newcommand{\authors}{\href{mailto:"Alistair Dunn"<alistair.dunn@niwa.co.nz>?subject=SAM:}{authors}} %check this
%hyper ref for email
\newcommand{\Organisation}{National Institute of Water \& Atmospheric Research Ltd.} %NIWA
\newcommand{\ManualRef}{Doonan. I.; Dunn, A.; Rasmussen, S.; Large, K.; Bull, B.; Francis, R.I.C.C.; McKenzie, A.; Gilbert, D.J.; Smith, M.H.; Bian, R.; Fu, D. (\DocYear) \SAMName\ User Manual, \SAM\ \VER. \Organisation\ \emph{Unpublished report}. \ref{TotPages} p.} % full document reference
\title{\SAMName\ User Manual \\\SAM\ \VER} %Document title
\author{I. Doonan, A. Dunn, S. Rasmussen, K. Large, B. Bull, R. I. C. C. Francis,\\ A. McKenzie, D. J. Gilbert, M. H. Smith, R. Bian, D. Fu} %Document authors %check this
...
% Title page
\pdfbookmark[1]{SAM Users Manual}{title}
...
% This is a rattail and will be replaced by Erika's logo
% As will the text in box that follows...
\begin{figure}[htp]
  \begin{center}
   \includegraphics[angle=50,height=4cm,width=3cm]{Figures/rattail}
  \end{center}
\end{figure}

% Some text to explain where KL at so far
% Define box style
\global\mdfdefinestyle{defKL}{%
linecolor=black,linewidth=4pt,%
leftmargin=1cm,rightmargin=1cm, %
backgroundcolor=lightgray
}
% Text in box
\begin{mdframed}[style=defKL]
\SAM\ logo being developed by Erika MacKay.\\\\
\SAM\ name tbd.\\\\
Confirmation of structure and contents order is required in the first instance.\\\\
Documentation copied and adapted from CASAL and SPM manuals - requres discussion, checking and rewriting.\\\\
Chapters for command and subcommand syntax are auto-generated 
from the \SAM\ code for Population, Estimation, Observation and
Report sections.\\\\
...
~\vfill
\begin{center}
  \SAMName\ User Manual (modified: \DocVer) \\ for use with \SAM\ \VER
\end{center}
...
\CH
\KL 4th level included for planning purposes, will be removed when order of contents confimed. \KLend
