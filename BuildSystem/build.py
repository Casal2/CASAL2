...
if os.system('which ' + compiler + ' > compiler.txt') != EX_OK:
...
if os.system('rm -rf compiler.txt') != EX_OK:
...
if operatingSystem == 'win32':
    if os.system('which cmd.exe > cmd.txt') != EX_OK:
    ...
    if os.system('rm -rf cmd.txt') != EX_OK:
...
if buildType.upper() == "THIRDPARTY":
    if os.system('python ' + cwd + '/buildtools/build_third_party.py') != EX_OK:
        sys.exit(1)
