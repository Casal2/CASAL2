...
class ModelRunner:
  ...
  def start(self):
    ...
    if not os.path.exists('bin/' + Globals.operating_system_ + '/release_betadiff/' + binary_name):
      ...
      print('Please build the release betadiff binary build before running the models')
      return False
    ...
    for folder in estimation_dir_list:
      ...
      if os.system("casal2 -e -g 20 > estimate.log 2>&1") != EX_OK:
        elapsed = time.time() - start
        print('[FAILED] - ' + folder + ' -e betadiff estimation run in ' + str(round(elapsed, 2)) + ' seconds')
        fail_count += 1
      else:
        elapsed = time.time() - start
        print('[OK] - ' + folder + ' -e betadiff estimation run in ' + str(round(elapsed, 2)) + ' seconds')
        success_count += 1
      os.chdir(cwd)
