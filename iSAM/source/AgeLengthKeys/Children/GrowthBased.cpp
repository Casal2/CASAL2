...
GrowthBased::GrowthBased() {
  parameters_.Bind<Double>(PARAM_CLASS_MINIMUMS, &class_mins_,
      "Length Class Minimums", "")->set_lower_bound(0.0, false);
  parameters_.Bind<string>(PARAM_DISTRIBUTION, &distribution_,
      "Distribution around each Age length mean", "")->set_allowed_values( {
      PARAM_NORMAL, PARAM_LOGNORMAL });
}

/** Estimates proportion of age class that is in each length interval
...
void GrowthBased::CummulativeNormal(
    ...
    vector<Double> class_mins, string distribution,
    vector<Double> Class_min_temp, bool plus_grp) {
