...
GrowthBased::GrowthBased() {
  data_table_ = TablePtr(new parameters::Table(PARAM_GROWTH_BASED));
  parameters_.Bind<Double>(PARAM_CLASS_MINS, &Class_mins_, "Length Class Minmums", "");
  parameters_.Bind<bool>(PARAM_PLUS_GROUP, &Plus_grp_, "Whether Plus Group is in age group", "");   // I feel that this can come from the config script in the partition or not
  parameters_.Bind<string>(PARAM_DISTRIBUTION, &Distribution_, "Distribution around each Age length mean","") // Ask Ian if this can come from @age_length command
  parameters_.Bind<Int>(PARAM_LENGTH_BINS, &Im2_, "Number of Length Bins","");
}

/**
 * validate the values passed in from the configuration
 * file. Check Class_mins are all positive & in succession & no ties &
 */
void GrowthBased::DoValidate(){
  for (unsigned i = 0; i <Im2_; ++i){
  	if(Class_mins_[i]<0.0)
  	LOG_ERROR_P(PARAM_CLASS_MINS) << " (" << AS_DOUBLE(Class_mins_[i]) << ") cannot be less than 0.0";
  	if(Class_mins_[i + 1] < Class_mins_[i]) // Something like that. this is going to error out because When i gets to the end there will be no i+1
  	LOG_ERROR_P(PARAM_CLASS_MINS) << "Length Classes must be in increasing order";
  	if(Class_mins_[i + 1] = Class_mins_[i]) // Same problm as earlier fix later this is a mino detail
  	LOG_ERROR_P(PARAM_CLASS_MINS) << "No tied values allowed";
...
//----------------------- Set up an internal function that will give the Cumulative normal probability 
Double GrowthBased::Cum_normal(
	...
	vector<Double>    class_mins,    //lower bounds of length classes
