...
public:
  void CummulativeNormal(Double mu, Double cv, vector<Double> *vprop_in_length, vector<Double> class_mins, string distribution, bool plus_grp) {
    VonBertalanffyTest x;
    x.CummulativeNormal(mu, cv, vprop_in_length, class_mins, distribution, plus_grp);
  }
...
TEST(AgeLength, VonBertalanffy_CummulativeNormal_1) {
  ...
  myTest.CummulativeNormal(mu, cv, &vprop_in_length, class_mins, distribution, plus_grp);
...
//  process->parameters().Add(PARAM_R0, "1000000", __FILE__, __LINE__);
