...
l_alpha <- 20
l_beta  <- 40
cv_val  <- 0.0 # 0.0 in CASAL file, 0.1 in Casal2 file - which one to use?
...
size_obj <- growth_curve_fn(pars = pars_tib, length_vec = lengths, nyears = 30, sex = 'male', initial_length = initial_size_mean, initial_cv = initial_size_cv)

all_size_obj <- bind_cols(size_obj, 'model' = 'CASAL')
...
pars_tib <- tibble('g1' = g_vec[1], 'g2' = g_vec[2], 'l1' = l_alpha, 'l2' = l_beta, 'cv' = cv_val, 'min_sigma' = min_sigma)
size_obj <- growth_curve_fn(pars = pars_tib, length_vec = lengths, nyears = 30, sex = 'female', initial_length = initial_size_mean, initial_cv = initial_size_cv)

all_size_obj <- bind_rows(all_size_obj, bind_cols(size_obj, 'model' = 'CASAL'))
...
all_size_obj <- bind_rows(all_size_obj, bind_cols(size_obj, 'model' = 'CASAL sens1'))
...
p <- all_size_obj %>%
        ...
        ggplot(aes(x = Year, y = mean_length, colour=factor(model))) +
        geom_line(stat="identity") + facet_wrap(sex ~ .)
