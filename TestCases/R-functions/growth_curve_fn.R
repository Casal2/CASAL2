# R function courtesy of Samik Datta and Vidette McGregor, NIWA
growth_curve_fn <- function(pars, length_vec=seq(0, 100), nyears=100, sex='male', initial_length = 1, initial_cv = 0.1){
  ...
  colnames(size_distribution) <- l_mid

  size_tib = tibble(time = factor(time_vec)) %>%
    bind_cols(as_tibble(size_distribution, .name_repair='check_unique')) %>%
