...
"plot.fits.casal2TAB" <- function(model, report_label = "", type = "resid", plot.it = T, xlim, ylim, xlab, ylab, main, col, ...) {
  ...
  if (!this_report$likelihood %in% likelihoods_allowed) {
    stop(paste0("This function can be used with likelihoods ", paste(likelihoods_allowed, collapse = ", "), " only."))
  }
  if (!this_report$observation_type %in% observations_allowed) {
    stop(paste0("This function can be used with observation types ", paste(observations_allowed, collapse = ", "), " only."))
  }
  ...
  if (plot.it) {
    if (this_report$likelihood == "lognormal") {
      ...
      if (n_years <= 4) {
        ...
        stop(paste0("There are ", n_years, " years of composition data. Set 'plot.it = FALSE' and use another function to plot these data."))
      }

      for (y in 1:n_years) {
        this_year <- this_pearson[, grepl(pattern = paste0("pearson_residuals\\[", years[y]), x = names(this_pearson))]
