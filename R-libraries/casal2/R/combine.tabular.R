...
#' @param Optionally, the Row number to burn in from. Note this is not the iteration but the row that corresponds to your iteration that you want to burn-in from. if keep > 1 then the iteration and row will be different
#' @param verbose print extra info.
#' @return a 'casal2TAB' object which has been manipulated.
#' @export
#'
"combine.tabular" <- function(tab_objects, Row = NULL, verbose = F) {
  ...
  for (i in 1:length(tab_objects)) {
    if(names(tab_objects[[1]]) != names(tab_objects[[i]]))
  ...
  for (i in 1:length(names(tab_object))) {
    ...
    for(j in 2:length(tab_objects)) {
      this_list$values <- rbind(this_list$values, get(names(tab_object)[i], tab_object)$values)
    }
    ## try and store it back in to the original object.
    tab_objects[[i]]$values <- this_list$values
  }
  return(tab_objects)
}
