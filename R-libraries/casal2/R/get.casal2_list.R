...
"get.casal2_list" <-
  ...
  mcmc_type = rep("vector",length(mcmc))
  ...
  ## Growth_model
  grow = c("a", "b", "units", "cv_first", "cv_last","distribution","casal_switch", "Linf","k","t0","length_weight","by_length","y1","y2","tau1","tau2","time_step_proportions","linf")
  ...
  ## priors 
  pri = c("mu","cv","sigma","lower_bound","upper_bound","multiplier","r","log_scale")
  pri_type = c(rep("vector",5),rep("single_value",3))
  ...
  ## Combine them all.
  casal2_list = list( command = c(model, categories, common, init, pro, obs, sel, grow, pri, tim, age, report,mcmc), type = c(model_type, categories_type, common_types, init_types, pro_type, obs_type, sel_type, grow_type, pri_type, tim_type, age_type, report_type,mcmc_type))
