...
"plot.derived_quantities.casal2MPD" = function(model, report_label = "", type = "number", xlim, ylim, xlab, ylab, main, col, plot.it = T, ...) {
  ...
  if (any(names(this_report) == "type")) {
    if (this_report$type != "derived_quantity")
      stop(paste0("The report label '", report_label, "' is not a derived_quantity. Please check that the correct report_label was specified."))
  ...
  if (!muliple_iterations_in_a_report) {
    ...
    #print(paste0("found - ",length(this_report), " iterations"))
    if (missing(col)) {
