...
The aim of this document is to describe and demonstrate the functionality in the Casal2 R package. This includes parsing output from deterministic and estimation \textit{Maximum Posterior Density} (MPD) runs, and MCMC runs. This could also serve as a best practices guide. The R package functions can interpret existing output only:  Casal2's reports are user-specified, i.e., to plot derived quantities in R, the \texttt{@report} for derived quantities must be defined in the model configuration files.
