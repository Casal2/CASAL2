...
The aim of this document is to describe and demonstrate the functionality in the Casal2 R package. This includes both deterministic and esitmation runs, and MCMC output. This could also serve as a best practices guide. The R library functions can interpret existing output only, and Casal2's reporting is user-defined, i.e., to plot derived quantities in R the \texttt{@report} for derived quantities must be defined in the model configuration files.
...
This document will demonstrate the use of the Casal2 R package for all three of these outputs. All files that are used in this demonstration are available in the \texttt{extdata} file in the \texttt{casal2} R package.
...
When Casal2 is either run deterministically (\texttt{casal2 -r}) or as an estimation run (\texttt{casal2 -e}), there are a range of options available for plotting derived quantities and parameter estimates of a model.
...
If this is an MPD run, view the warnings and the minimiser's convergence status. This information will always be output for an estimation run.
...
If the Casal2 model configuration reports either normalised or Pearson's residuals for any of the obsevations, plot the residuals against the theoretical standard normal quantiles.
...
This can also be done for multinomial Pearson's residuals, with them and both normalised residuals \(\mathcal{N}(0,1)\)

Once the model fits reasonably to the data, look at the derived quantities and parameter estimates. The Casal2 R package can plot these using the $`plot.derived_quantities()`$ function.
...
The function can return the SSBs only by setting the input \texttt{plot.it = F}, and then create another plot.
...
Other derived quantities include selectivities
...
Next, check that the minimum reported is reached with multiple parameter values.
...
Casal2 has inbuild data weighting functions for relative indices of abundance or biomass with the \texttt{CV.for.CPUE} function. This function generates CVs for a relative biomass index based on loess smoother. There is also the Francis method TA1.8 [@francis2011data] \texttt{Method.TA1.8()} demonstrated below.
...
There are also wrapper functions that can help automating the dataweighting procedure.
...
If setting up a model for estimating the \textit{Maximum Posterior Density} (MPD), an important consideration is whether the model has been estimated at a local or global minimum. One method available to check for a global minimum is to run multiple estimations from different starting values. One function available for generating multiple starting values is the \texttt{generate.starting.pars()} function. This function takes a text configuration file, and parses the \texttt{@estimate} blocks to find the prior and lower and upper bounds for estimated parameters.
...
This function will create the file $`random\_start.out`$ and format the values so they are compatible with $`casal2 -e -i random_start.out > multi_start_mpd.out`$ format. A note when using this function: this function may generate implausable values if the bounds are wide, so more consideration is needed than when setting bounds for a model. This will create a multi run output, with an example below.
...
A function to identify parameters that are unidentfiable is $`check_mpd_identifiability()`$. Set up the model configuration files to report the covariance matrix and the estimated parameters in order for this function to work.
...
There are two formats for MCMC outputs, the objective function values and parameter samples, and the derived quantitites. The first type of output is created with the \texttt{casal2 -m} command, and the latter is created with \texttt{casal2 -r --tabular -i mcmc\_samples.out}.
