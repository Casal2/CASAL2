...
Once you are satisfied with the goodness of fit of the model, then you will want to look at derived quantities and parameters. The R library can do a very basic plot using the \texttt{plot.derived_quantities()} function.
...
## Plotting priors
It is usefult to visulise priors, that are assumed in a Casal2 assessment model. We have added the \texttt{plot.prio()} function from the casal R package so this can be easily done.
...
## Dataweighting 
Casal2 has inbuild data weighting functions, such as \texttt{ CV.for.CPUE} which is a method that associates cv's for a relative biomass trend, based on loess smoothness. There is also the Francis Method TA1.8 \texttt{Method.TA1.8()} demonstrated below.


```{r dataweighting, linewidth=60, include=TRUE, eval=T}
Method.TA1.8(model = single_run, observation_labels = c("tan_at_age"), plot.it = T)
```
Assess MPD comvergence
...
This function will create the file \texttt{random\_start.out} and format the values so they are compatible with \texttt{casal2 -e -i random\_start.out > multi_start_mpd.out} format. This will create a multi run output which is given below for an example of using
...
Another helpful function that is available to identify parameters that are unidentfiable is \texttt{check_mpd_identifiability()}. You will need to report the covariance matrix and the estimate values in order for this function to work.
...
```{r read_mcmc, linewidth=60, eval=FALSE}
# this will create a list like object called single_run
mcmc_out = extract.mcmc(samples.file = "mcmc_samples.out",objectives.file = "mcmc_objectives.out",
...
mcmc_out = extract.mcmc(samples.file = "mcmc_samples.out",objectives.file = "mcmc_objectives.out",return_covariance = T, path = fpath)
# convert to coda mcmc object
# drop out sample jacobians, step_size, acceptance_rate, acceptance_rate_since_adapt
mcmc_chain = as.mcmc(mcmc_out$Data[, !colnames(mcmc_out$Data) %in% c("sample", "jacobians", 
                                                                     "step_size", "acceptance_rate", "penalties", "acceptance_rate_since_adapt")])
