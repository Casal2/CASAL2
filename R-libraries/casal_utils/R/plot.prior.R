"plot.prior"<-
function(type="lognormal",mu,sd,cv,A=0,B=1,bounds,xlim,label=F,xlab="x",ylab="Density",logx=F,add=F,dump=F,...)
{
  ...
  n<-201
  if(missing(xlim) &!missing(bounds)) xlim<-bounds
  else if(missing(bounds) &!missing(xlim)) bounds<-xlim
  ...
  if(type=="") stop(paste("Prior not found. Available priors are\n",paste(Allowed,collapse=", ")))
  else cat(paste("Plotting ",type," prior\n",sep=""))
  if(logx) p<-exp(seq(log(bounds[1]),log(bounds[2]),length=n))
  else p<-seq(bounds[1],bounds[2],length=n)
  if(type=="lognormal") {
    ...
    if(logx) res<-p
    else res<-rep(1,length(p))
  } else  if(type=="uniform-log") {
    if(logx) res<- rep(1,length(p))
    else res<-exp(-log(p))
